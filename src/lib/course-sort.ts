...
export function getCourseSortOrder(course: CourseSortInput): number {
  ...
  if (/\btef\b|tef[\s_-]canada/.test(haystack)) return 1;
  if (/\btcf\b|tcf[\s_-]canada/.test(haystack)) return 2;
  if (/\ba1\b|beginner/.test(haystack)) return 3;
  if (/\ba2\b|elementary/.test(haystack)) return 4;
  if (/\bb1\b/.test(haystack)) return 5;
  if (/\bb2\b|upper\s*intermediate/.test(haystack)) return 6;

  if (/intermediate/.test(haystack) && !/upper/.test(haystack)) return 5;
  if (/advanced|upper/.test(haystack)) return 6;
